        ...
        <v-card class="pa-4">
          ...
          <v-card-item>
            <v-form ref="refForm" novalidate @submit.prevent="submit">
              <v-text-field v-model="registerRequest.firstName" label="First Name" prepend-inner-icon="mdi-account"
                :rules="[requiredValidator]"></v-text-field>

              <v-text-field v-model="registerRequest.lastName" label="Last Name" prepend-inner-icon="mdi-account"
                :rules="[requiredValidator]"></v-text-field>

              <v-text-field v-model="registerRequest.email" label="Email" prepend-inner-icon="mdi-email"
                :rules="[requiredValidator, emailValidator]"></v-text-field>

              <v-text-field v-model="registerRequest.password"
                :append-inner-icon="isPasswordVisible ? 'mdi-eye-off-outline' : 'mdi-eye-outline'" label="Password"
                prepend-inner-icon="mdi-key" :rules="[passwordValidator]"
