    ...
    <v-container>
        <v-row v-for="(selectedBet, i) in playingTicketStore.selectedBets" :key="i" no-gutters>
            ...
            <v-col cols="9">
                ...
                <p class="bet-text"><span class="font-weight-regular">{{ selectedBet.SelectedBetType.title }}</span> -
                    <span class="text-red-darken-1">{{ selectedBet.SelectedBetType.quota }}</span></p>
            </v-col>
        </v-row>
    </v-container>
    <hr />
    <v-container>
        <v-row v-if="playingTicketStore.selectedBets.length" no-gutters>
            <v-col class="text-right" cols="12">
                <v-text-field v-model.number="ticketAmount" density="compact" hide-details="auto" label="Ticket Amount"
                    prefix="€" step="0.1" type="number">
                </v-text-field>
            </v-col>
            ...
            <v-col cols="12">
                <v-btn block class="mt-2" color="green-accent-2" variant="elevated" @click="playTicket">
                    Play
                </v-btn>
            </v-col>
            <v-col cols="12">
                <v-btn block class="mt-2" color="red-darken-1" variant="elevated"
